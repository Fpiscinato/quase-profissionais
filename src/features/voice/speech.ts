...
// Remembers the last thing spoken so the "Repita" / "Repeat" voice command
// (see useVoiceCommands) can say it again without the caller needing to
// track its own history.
let lastSpoken: { lang: Lang; text: string } | null = null
 ...
 */
export function speak(lang: Lang, text: string): void {
  if (!text) return
  lastSpoken = { lang, text }
...
/** Re-speaks whatever was last announced — the "Repita"/"Repeat" voice command. */
