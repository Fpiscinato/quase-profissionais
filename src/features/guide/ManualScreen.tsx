...
const SECTIONS = [
  ...
  {
    title: '3. Jogue as rodadas',
    body: 'A tela "Rodadas" lista os confrontos do dia. Toque em "Configurar partida" pra definir quem saca primeiro, depois "Iniciar partida" pra abrir a tela ao vivo.',
  },
  {
    title: '4. Tela ao vivo',
...
export function ManualScreen() {
  return (
    <div className="flex flex-col gap-4 p-4">
      <div>
        <h1 className="text-xl font-bold">Como usar o app</h1>
        <p className="text-sm text-cream/70">
          Um passo a passo rápido, do cadastro dos jogadores até o fim do torneio. Tudo funciona
          offline, direto no seu aparelho.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-3">
        {SECTIONS.map((s) => (
          <div key={s.title} className={card}>
            <h2 className="mb-1 font-bold text-lime">{s.title}</h2>
            <p className="text-sm text-cream/85">{s.body}</p>
          </div>
        ))}
      </div>
    </div>
