...
export const EN: Record<string, string> = {
  ...
  'Jogador com o controle remoto ou relógio': 'Player holding the remote control or watch',
  'Se alguém vai marcar os pontos pelo controle físico ou pelo relógio, escolha quem é aqui — o time dele fica sempre fixado como Time 1 ou Time 2, trocando os times automaticamente se precisar, pra os botões nunca mudarem de lugar de uma rodada pra outra. Fica lembrado no aparelho pras próximas partidas.':
    "If someone is going to score points with the physical remote or the watch, pick who here — their team always stays fixed as Team 1 or Team 2, swapping teams automatically when needed, so the buttons never move from one round to the next. Remembered on this device for future matches.",
  Nenhum: 'None',
  ...
  Tiebreak: 'Tiebreak',
  'Quando os games empatam no topo do set (ex.: 4-4), joga-se um desempate até 7 pontos, precisando de 2 de vantagem, pra decidir o set. Quem sacaria o próximo game saca sozinho o 1º ponto do tiebreak; dali em diante o saque troca de time a cada 2 pontos. A troca de lado acontece a cada 6 pontos somados (6, 12, 18...) e de novo ao final do tiebreak, antes do próximo set começar.':
    'When games tie at the top of the set (e.g. 4-4), a tiebreak up to 7 points, winning by 2, decides the set. Whoever would have served the next game serves the tiebreak\'s 1st point alone; from then on, serve switches teams every 2 points. Ends change every 6 combined points (6, 12, 18...) and again at the end of the tiebreak, before the next set starts.',
  'Termos do tênis': 'Tennis terms',
  ...
  'Jogando pelo relógio': 'Playing via the watch',
  'Dá pra marcar os pontos direto do pulso (Galaxy Watch), pareando em "Configurar relógio". Na tela do relógio, o ícone do app fica entre Desfazer e Repetir — tocar nele pede confirmação e troca o código, útil pra desparear ou trocar de aparelho. Sem conexão confirmada os botões ficam travados, pra não marcar ponto à toa. Detalhes completos em "Como usar".':
    'You can score points straight from your wrist (Galaxy Watch), by pairing in "Set up watch". On the watch screen, the app icon sits between Undo and Repeat — tapping it asks for confirmation and changes the code, useful to unpair or switch devices. Without a confirmed connection the buttons stay locked, so you don\'t score points into the void. Full details in "How to use".',
  ...
  'Em "Controle remoto" você mapeia os botões de um controle físico (ex. um "clicker" de apresentação USB) pra Ponto Time 1, Ponto Time 2, Repetir anúncio, Desfazer e Salvar partida. Toque em "Definir", aperte o botão do controle, pronto — funciona com qualquer controle que emule teclado, não só um modelo específico. A tela ao vivo escuta esses botões o tempo todo, mesmo com o Modo viva-voz desligado. Se quem aperta o controle (ou usa o relógio, seção 12) se confunde com a troca de times a cada rodada, use "Jogador com o controle remoto ou relógio" em "Configurar partida" pra fixar o time dele sempre no mesmo botão.':
    'In "Remote control" you map a physical remote\'s buttons (e.g. a USB presentation "clicker") to Point Team 1, Point Team 2, Repeat announcement, Undo and Save match. Tap "Set", press the remote\'s button, done — works with any device that emulates a keyboard, not just one specific model. The live screen listens for these buttons the whole time, even with Hands-free voice mode off. If whoever holds the remote (or uses the watch, section 12) gets confused by teams swapping each round, use "Player holding the remote control or watch" in "Set up match" to fix their team to the same button every time.',
  // WatchSetupScreen
  ...
  'O relógio só mostra o placar de verdade com a tela "Partida ao vivo" aberta aqui no app — nesta tela ele só pareia.':
    'The watch only shows the real score while the "Live match" screen is open here in the app — this screen only pairs it.',
  ...
  '12. Relógio (Wear OS)': '12. Watch (Wear OS)',
  'Quem estiver com o Galaxy Watch pode marcar os pontos direto do pulso, sem carregar o controle remoto pendurado no pescoço. Em "Configurar relógio" gere um código de 6 dígitos e escolha se a tela do relógio fica sempre ativa (padrão) ou escurece sozinha depois de um tempo parada, pra economizar bateria. No navegador do relógio, abra o endereço indicado ali e digite o código num teclado numérico grande — o placar (0/15/30/40), quem saca e o lado aparecem sozinhos, atualizados pelo aparelho principal. Sem conexão confirmada, os botões do relógio ficam travados (não marcam ponto à toa) até reconectar. Tocar no ícone do app no relógio (entre Desfazer e Repetir) pede confirmação e troca o código — use se precisar desparear ou conectar em outro aparelho. Sem Wi-Fi na quadra? Ative o ponto de acesso do tablet/celular e conecte o Wi-Fi do relógio nele (não é o Bluetooth de sempre) — funciona igual. Se quem usa o relógio se confunde com a troca de times a cada rodada, use "Jogador com o controle remoto ou relógio" em "Configurar partida" (seção 4). Só relógios Samsung/Wear OS por enquanto; Apple Watch não tem navegador de internet e ainda não é suportado.':
    'Whoever is wearing the Galaxy Watch can score points straight from their wrist, no remote control hanging off their neck. In "Set up watch" generate a 6-digit code and choose whether the watch screen stays always on (default) or dims on its own after a while idle, to save battery. In the watch\'s browser, open the address shown there and type the code on a big number pad — the score (0/15/30/40), who serves and which side appear on their own, updated by the main device. Without a confirmed connection, the watch buttons stay locked (no scoring points into the void) until it reconnects. Tapping the app icon on the watch (between Undo and Repeat) asks for confirmation and changes the code — use it if you need to unpair or connect a different device. No Wi-Fi at the court? Turn on the tablet/phone\'s hotspot and connect the watch\'s own Wi-Fi to it (not the usual Bluetooth) — works the same way. If whoever uses the watch gets confused by teams swapping each round, use "Player holding the remote control or watch" in "Set up match" (section 4). Samsung/Wear OS watches only for now; Apple Watch has no general web browser and isn\'t supported yet.',
  ...
  Apelido: 'Nickname',
  'Apelido de até 4 letras': 'Nickname, up to 4 letters',
  'Usado nas telas pequenas, como o relógio — se não preencher, o app usa as 4 primeiras letras do nome.':
    'Used on small screens, like the watch — if left blank, the app uses the first 4 letters of the name.',
  'O apelido pode ter no máximo 4 letras.': 'The nickname can be at most 4 letters long.',
  ...
  'Incluir tempo de cada partida ao compartilhar': 'Include each match\'s time when sharing',
  Compartilhar: 'Share',
  'Gerando...': 'Generating...',
  ...
  'Tempo de cada partida': 'Time per match',
