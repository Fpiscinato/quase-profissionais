...
export const EN: Record<string, string> = {
  ...
  'Em "Controle remoto" você mapeia os botões de um controle físico (ex. um "clicker" de apresentação USB) pra Ponto Time 1, Ponto Time 2, Repetir anúncio, Desfazer e Salvar partida. Toque em "Definir", aperte o botão do controle, pronto — funciona com qualquer controle que emule teclado, não só um modelo específico. A tela ao vivo escuta esses botões o tempo todo, mesmo com o Modo viva-voz desligado. Se quem aperta o controle (ou usa o relógio, seção 13) se confunde com a troca de times a cada rodada, use "Jogador com o controle remoto ou relógio" em "Configurar partida" pra fixar o time dele sempre no mesmo botão.':
    'In "Remote control" you map a physical remote\'s buttons (e.g. a USB presentation "clicker") to Point Team 1, Point Team 2, Repeat announcement, Undo and Save match. Tap "Set", press the remote\'s button, done — works with any device that emulates a keyboard, not just one specific model. The live screen listens for these buttons the whole time, even with Hands-free voice mode off. If whoever holds the remote (or uses the watch, section 13) gets confused by teams swapping each round, use "Player holding the remote control or watch" in "Set up match" to fix their team to the same button every time.',
  // WatchSetupScreen
